...
from darts.models import ARIMA, RegressionModel, NLinearModel
...
def forecast_usage(df_1, df_2, df_3):
    new_dfs = []
    for df in [df_1, df_2, df_3]:
        df = df.sort_values(by=['Date', 'Arrival'])

        df = df[['Date', 'Day of Week', 'Onboard']]
        grouped = df.groupby('Date').agg({'Onboard': 'mean', 'Day of Week': 'first'}).reset_index()
        grouped['Date'] = pd.to_datetime(grouped['Date'])
        grouped.set_index('Date', inplace=True)
        full_date_range = pd.date_range(start=grouped.index.min(), end=grouped.index.max(), freq='B')
        ...
        df_reindexed['Onboard'] = df_reindexed['Onboard'].interpolate(method='linear')
        df_reindexed.reset_index(inplace=True)
        df_reindexed = df_reindexed.rename(columns={'index': 'Date'})
        df_reindexed['day'] = df_reindexed['Day of Week'].map(
            {'Monday': 1, 'Tuesday': 2, 'Wednesday': 3, 'Thursday': 4, 'Friday': 5})
    ...
    spring_series = TimeSeries.from_dataframe(spring, 'Date', 'Onboard', freq='B', fill_missing_dates=False)
    fall_series = TimeSeries.from_dataframe(fall, 'Date', 'Onboard', freq='B', fill_missing_dates=False)
    pilot_train_series = TimeSeries.from_dataframe(train_pilot, 'Date', 'Onboard', freq='B', fill_missing_dates=False)
    pilot_test_series = TimeSeries.from_dataframe(test_pilot, 'Date', 'Onboard', freq='B', fill_missing_dates=False)
    ...
    #model = ARIMA(q=0, d=1, p=8)
    #model = RegressionModel(model=HistGradientBoostingRegressor(), lags=15, lags_future_covariates=(15,5), output_chunk_length=5)
    model = NLinearModel(input_chunk_length=20, output_chunk_length=5, n_epochs=50)
    model.fit(spring_series, future_covariates=spring_covariates)
    model.fit(fall_series, future_covariates=fall_covariates)
    model.fit(pilot_train_series, future_covariates=pilot_covariates)
    forecast = model.predict(len(test_pilot), future_covariates=pilot_covariates)
    ...
    plt.xticks(rotation=30, fontsize=12)
    plt.yticks(fontsize=12)
    plt.ylabel('Normalized Average Onboard Counts', fontsize=16)
    plt.title('Forecasting Onboard Counts by Day- RMSE: {:.4f}'.format(error), fontsize=20)
    plt.savefig('forecasting onboard.png')
    plt.clf()
    plt.cla()

    plt.figure(figsize=(14, 8))
    plt.plot(spring['Date'], spring['Onboard'], label='Spring 2023')
    plt.plot(fall['Date'], fall['Onboard'], label='Fall 2023')
    plt.plot(pilot['Date'], pilot['Onboard'], label='Spring 2024')
    plt.gca().xaxis.set_major_locator(mdates.MonthLocator())
    plt.legend(fontsize=16)
    plt.xticks(rotation=30, fontsize=12)
    plt.yticks(fontsize=12)
    plt.ylabel('Average Onboard Count', fontsize=16)
    plt.title('Onboard Count by Semester', fontsize=20)
    plt.savefig('summary onboard.png')
    plt.clf()
    plt.cla()
...
df_1 = pd.read_excel('usage.xlsx', sheet_name='Spring2023')
df_2 = pd.read_excel('usage.xlsx', sheet_name='Fall2023')
df_3 = pd.read_excel('usage.xlsx', sheet_name='Spring2024')

forecast_usage(df_1, df_2, df_3)